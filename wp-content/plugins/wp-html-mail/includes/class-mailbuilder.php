...
final class Haet_Mail_Builder {
	...
	public function render_header_meta_box() {
		...
		<div class="haet-mailbuilder-header clearfix">
			<h4 class="email-name">
		<?php echo esc_html( str_replace( '_', ' ', str_replace( 'WC_Email_', '', get_the_title() ) ) ); ?>
			</h4>
			<div class="header-buttons">
		<?php do_action( 'haet_mailbuilder_header_buttons', $post ); ?>
				<button type="submit" class="button button-primary"><?php esc_html_e( 'Update' ); ?></button>
			</div>
		</div>
		<?php
	}
	...
	public function render_subject_meta_box() {
		...
		echo '<input type="text" name="subject" value="' . esc_attr( htmlentities( $value ) ) . '">';
	}
	...
	public function render_attachments_meta_box() {
		...
		<div class="uploader">
			<input id="mailbuilder_attachments" name="mailbuilder_attachments" type="hidden" value='<?php echo esc_attr( $attachments ); ?>' />
			<button class="button upload_attachment_button" type="button">
				<span class="dashicons dashicons-paperclip"></span>
				<?php esc_attr_e( 'Add attachments', 'haet_mail' ); ?>
			</button>
		</div>
		<?php
	}
	...
	private function output_template_styles_for_mailbuilder( $template, $custom_css_desktop = '', $custom_css_mobile = '' ) {
		...
		if ( count( $output_array ) > 1 && count( $output_array[1] ) > 0 ) {
			...
			// phpcs:disable
			echo '<style>' . strip_tags( $css_object->render() ) . '</style>';
			// phpcs:enable
		}
	}
	...
	private function output_template_header_for_mailbuilder( $template ) {
		...
		if ( count( $output_array ) > 1 && count( $output_array[1] ) > 0 ) {
			...
			// I didn't find a way to escape HTML.
			// phpcs:disable
			echo $header;
			// phpcs:enable
		}
	...
	public function validate_css( $css, $prettify = false ) {
		$css = strip_tags( $css );
		include_once HAET_MAIL_PATH . '/vendor/autoload.php';
	...
	public function render_mailbuilder_meta_box() {
		...
		<input type="hidden" name="mailbuilder_json" id="mailbuilder_json" value='<?php echo esc_attr( $value ); ?>'>
		<input type="hidden" name="mailbuilder_header_hidden" id="mailbuilder_header_hidden" value="<?php echo esc_attr( $header_hidden ); ?>">
		<input type="hidden" name="mailbuilder_footer_hidden" id="mailbuilder_footer_hidden" value="<?php echo esc_attr( $footer_hidden ); ?>">
		<div id="mailbuilder-editor">
			...
			<div class="container">
				<div id="mailbuilder-header" class="<?php echo ( $header_hidden ? 'mailbuilder-hidden' : '' ); ?>">
					...
					<a href="#" class="mailbuilder-hide-button" data-status-field="mailbuilder_header_hidden">
						...
						<?php esc_html_e( 'show / hide header for this email', 'wp-html-mail' ); ?>
					</a>
				</div>
				...
				<div id="mailbuilder-footer" class="<?php echo ( $footer_hidden ? 'mailbuilder-hidden' : '' ); ?>">
					...
					<a href="#" class="mailbuilder-hide-button" data-status-field="mailbuilder_footer_hidden">
						<span class="dashicons dashicons-visibility"></span>
						<span class="dashicons dashicons-hidden"></span>
						<?php esc_html_e( 'show / hide footer for this email', 'wp-html-mail' ); ?>
					</a>
				</div>
			</div>
			<br><br><br>
			...
			<a href="#" class="mailbuilder-custom-css-button">
				...
				<?php esc_html_e( 'Custom CSS', 'wp-html-mail' ); ?>
			</a>
		</div>
		...
		?>
			...
			<div id="mb-css-edit" class="mailbuilder-sidebar-element">
				<h3><?php esc_html_e( 'Custom CSS', 'wp-html-mail' ); ?></h3>
				<p class="description"><?php esc_html_e( 'Warning: You are adding custom code here which is sent to your site visitors. Wrong code can destroy your emails or can even be a security risk. Only use this feature if you know what you are doing here!', 'wp-html-mail' ); ?></p>
				<?php
				...
				// phpcs:enable
				?>
				<p class="description"><?php esc_html_e( 'WP HTML Mail converts all desktop styling to inline CSS code for better support in webmail clients. Mobile CSS stays in the head, so please don\'t write @media queries to the field above but use the mobil CSS field below', 'wp-html-mail' ); ?></p>
				<h3><?php esc_html_e( 'Mobile CSS', 'wp-html-mail' ); ?></h3>
				<?php
				// already validated before.
				// phpcs:disable
				?>
				...
				// phpcs:enable
				?>
				<p class="description"><?php esc_html_e( 'CSS changes are not shown live. Please save the email to see your changes.', 'wp-html-mail' ); ?></p>

				<div class="mb-popup-buttons">
					<button class="mb-apply button button-primary" type="button">
						<span class="dashicons dashicons-yes"></span>
						<?php esc_html_e( 'Done', 'wp-html-mail' ); ?>
					</button>
				</div>
			</div>
	...
	public function validate_mailbuilder_element_settings( $settings ) {
		// each setting can either be a string or another array.
		if ( is_array( $settings ) ) {
			// go one level deeper.
			foreach ( $settings as $setting_key => $setting ) {
				$settings[ $setting_key ] = $this->validate_mailbuilder_element_settings( $setting );
			}
			return $settings;
		} else {
			// reached a value.
	...
	public function validate_mailbuilder_element_contents( $contents ) {
		// each content can either be a string or another array.
		if ( is_array( $contents ) ) {
			// go one level deeper.
			foreach ( $contents as $content_key => $content ) {
				$contents[ $content_key ] = $this->validate_mailbuilder_element_contents( $content );
			}
			return $contents;
		} else {
			// reached a value.
			return strip_tags( $contents, '<h1><h2><h3><h4><h5><blockquote><center><p><a><div><b><strong><i><em><button><table><thead><tbody><tr><th><td><span><br><hr><img><style><outlookconditionalcommentstart><outlookconditionalcommentend>' );
		}
	}
	...
	public function validate_mailbuilder_json( $mailbuilder_json ) {
		$mailbuilder_array = json_decode( stripslashes( $mailbuilder_json ), true );

		if ( is_array( $mailbuilder_array ) && count( $mailbuilder_array ) ) {
			foreach ( $mailbuilder_array as $element_id => $element_content ) {
				$mailbuilder_array[ $element_id ]['type']     = sanitize_key( $mailbuilder_array[ $element_id ]['type'] );
				$mailbuilder_array[ $element_id ]['id']       = sanitize_key( $mailbuilder_array[ $element_id ]['id'] );
				$mailbuilder_array[ $element_id ]['content']  = $this->validate_mailbuilder_element_contents( $mailbuilder_array[ $element_id ]['content'] );
				$mailbuilder_array[ $element_id ]['settings'] = $this->validate_mailbuilder_element_settings( $mailbuilder_array[ $element_id ]['settings'] );
			}
			return wp_json_encode( $mailbuilder_array );
		}
	...
	public function save_post( $post_id ) {
		...
		if ( ! current_user_can( 'edit_posts' ) ) {
			...
			update_post_meta( $post_id, 'mailbuilder_json', $mailbuilder_json );
		}

		if ( isset( $_POST['subject'] ) ) {
			update_post_meta( $post_id, 'subject', sanitize_text_field( wp_unslash( $_POST['subject'] ) ) );
		}

		if ( isset( $_POST['mailbuilder_attachments'] ) ) { // just a list of ids.
			update_post_meta( $post_id, 'mailbuilder_attachments', sanitize_text_field( wp_unslash( $_POST['mailbuilder_attachments'] ) ) );
		}

		if ( isset( $_POST['mailbuilder_css_desktop'] ) ) {
			update_post_meta( $post_id, 'mailbuilder_css_desktop', $this->validate_css( $_POST['mailbuilder_css_desktop'] ) );
		}

		if ( isset( $_POST['mailbuilder_css_mobile'] ) ) {
			update_post_meta( $post_id, 'mailbuilder_css_mobile', $this->validate_css( $_POST['mailbuilder_css_mobile'] ) );
		}

		if ( isset( $_POST['mailbuilder_header_hidden'] ) ) {
			update_post_meta( $post_id, 'mailbuilder_header_hidden', sanitize_text_field( wp_unslash( $_POST['mailbuilder_header_hidden'] ) ) );
		}

		if ( isset( $_POST['mailbuilder_footer_hidden'] ) ) {
			update_post_meta( $post_id, 'mailbuilder_footer_hidden', sanitize_text_field( wp_unslash( $_POST['mailbuilder_footer_hidden'] ) ) );
		}
	}
	...
	private function print_add_content_menu() {
		...
		<div class="mb-add-wrap mailbuilder-sidebar-element active">
			<h4>
				<?php esc_html_e( 'Add Content Element', 'wp-html-mail' ); ?>
			</h4>
		...
		<?php foreach ( $content_types as $content_type ) : ?>
					<li>
						<a href="#" data-type="<?php echo esc_attr( $content_type['name'] ); ?>" <?php echo ( $content_type['once'] ? 'data-once="once"' : '' ); ?>>
						<?php echo ( false !== strpos( $content_type['icon'], 'dashicons-' ) ? '<span class="dashicons ' . esc_attr( $content_type['icon'] ) . '"></span>' : '<img class="mb-type-icon" src="' . esc_url( $content_type['icon'] ) . '">' ); ?>
						<?php echo esc_html( $content_type['nicename'] ); ?>
						</a>
					</li>
		<?php endforeach; ?>
			</ul>
	...
	public function get_email_post_id( $email_name ) {
		...
		if ( $email_post_id && Haet_Mail()->multilanguage->is_multilanguage_site() && ! $translated_email_post_id ) {
			...
			if ( Haet_Mail()->multilanguage->get_multilang_plugin() === 'WPML' ) {
				...
				if ( ! $translations ) {
					$lang = PLL()->model->post->get_language( $email_post_id );
					if ( $lang ) {
	...
	public function customize_editor_toolbar( $init_array ) {
		global $post;
		if ( $post ) {
			$post_type = get_post_type( $post->ID );
			if ( $post_type === 'wphtmlmail_mail' && '#mb_wysiwyg_editor' === $init_array['selector'] ) {
				$init_array['block_formats'] = 'Headline=h1;Subheadline=h2;Paragraph=p;';
				...
				$init_array['toolbar1'] = 'formatselect,fontselect,fontsizeselect,|,bold,italic,|,alignleft,aligncenter,alignright,|,pastetext,removeformat,|,undo,redo,|,bullist,numlist,|,link,unlink,forecolor,|,spellchecker,fullscreen,table,code';
				// Font size.
				$init_array['fontsize_formats'] = '10px 11px 12px 14px 16px 18px 20px 22px 24px 28px 32px';
	...
	public function print_email( $email_name, $settings ) {
		...
		if ( null !== $mailbuilder_array ) :
			echo '<!--mailbuilder[' . esc_attr( $email_name ) . ']-->';
