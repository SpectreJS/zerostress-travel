...
if ( ! class_exists( '\WSAL\WP_Sensors\WP_Meta_Data_Sensor' ) ) {
	/**
	...
	class WP_Meta_Data_Sensor {
		...
		public static function event_post_meta_created( $object_id, $meta_key, $meta_value ) {
			...
			if ( $log_meta_event ) {
				$editor_link = WP_Helper::get_editor_link( $post );
		...
		public static function event_post_meta_updated( $meta_id, $object_id, $meta_key, $meta_value ) {
			 ...
			 * @since 3.2.2
			 */
			do_action( 'wsal_post_meta_updated', $meta_id, $object_id, (object) self::$old_meta, $meta_key, $meta_value );

			if ( isset( self::$old_meta[ $meta_id ] ) ) {
				/**
				 ...
				 * @since 3.3.1
				 */
				$log_meta_event = apply_filters( 'wsal_before_post_meta_update_event', true, $meta_key, $meta_value, self::$old_meta[ $meta_id ], $post, $meta_id );
		...
		public static function must_not_contain_role_changes() {
			return ! Alert_Manager::will_or_has_triggered( 4002 );
		}
		...
		public static function must_not_contain_new_user_alert() {
			return ! Alert_Manager::will_or_has_triggered( 4001 ) && ! Alert_Manager::will_or_has_triggered( 4012 );
		}
