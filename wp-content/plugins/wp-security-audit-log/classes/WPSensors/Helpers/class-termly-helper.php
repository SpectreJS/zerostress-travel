...
if ( ! class_exists( '\WSAL\WP_Sensors\Helpers\Termly_Helper' ) ) {
	/**
	 * Helper Sensor class for Termly.
	 *
	 * @package    wsal
	...
	class Termly_Helper {
		 ...
		 * @since 5.4.0
		 */
		public static function wsal_termly_add_custom_event_objects( $objects ) {
			$new_objects = array(
				'termly' => esc_html__( 'Termly', 'wp-security-audit-log' ),
			);
		...
		public static function load_alerts_for_sensor(): bool {
			if ( null === self::$plugin_active_for_sensors ) {
				self::$plugin_active_for_sensors = WP_Helper::is_plugin_active( 'uk-cookie-consent/uk-cookie-consent.php' );
			}
		 ...
		 * @since 5.4.0
		 */
		public static function wsal_termly_add_custom_event_type( $types ) {
			// $new_types = array(
