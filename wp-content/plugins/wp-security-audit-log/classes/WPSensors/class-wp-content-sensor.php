...
if ( ! class_exists( '\WSAL\WP_Sensors\WP_Content_Sensor' ) ) {
	/**
	...
	class WP_Content_Sensor {
		...
		public static function event_post_deleted( $post_id ) {
			...
			Alert_Manager::trigger_event( $event, $event_data ); // Log event.
		}
		...
		public static function check_template_change( $post_id, $meta_value ) {
			$post     = get_post( $post_id );
			$old_tmpl = ( self::$old_tmpl && 'page' !== basename( self::$old_tmpl, '.php' ) ) ? ucwords( str_replace( array( '-', '_' ), ' ', basename( self::$old_tmpl, '.php' ) ) ) : __( 'Default template', 'wp-security-audit-log' );
			...
			if ( $oldpost->post_status !== $newpost->post_status ) {
				...
				if ( $event ) {
					...
					if ( $is_scheduled ) {
						...
						Alert_Manager::trigger_event_if( $event, $event_data, array( __CLASS__, 'plugin_not_created_post' ) );
					}
				}
		...
		private static function check_categories_change( $old_cats, $new_cats, $post ) {
			...
			if ( $old_cats !== $new_cats && 'page' !== $post->post_type ) {
				...
				Alert_Manager::trigger_event( 2016, $alert_data );
			}
		}
		...
		public static function plugin_not_created_post() {
			...
			$triggered = Alert_Manager::will_or_has_triggered( 5019 );
